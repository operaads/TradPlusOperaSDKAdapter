...
static NSString * const kSplashC2SPlacementId = @"C55ED34A79B0333CDBFBF4BB99F89522";
static NSString * const kOpAdxTradPlusC2SPriceEventNotification = @"OpAdxTradPlusC2SPriceEventNotification";

@interface SplashC2SBidingVC ()<TradPlusADSplashDelegate>
...
@implementation SplashC2SBidingVC
...
- (void)loadAd {
    ...
    [self appendFlowLog:@"点击 Load，开始请求竞价开屏"];
    [self.splashAd loadAdWithWindow:[UIApplication sharedApplication].keyWindow bottomView:nil];
}

- (void)showAd {
    [self appendFlowLog:[NSString stringWithFormat:@"点击 Show，isAdReady=%@", self.splashAd.isAdReady ? @"YES" : @"NO"]];
    if (self.splashAd.isAdReady) {
        [self.splashAd show];
        [self appendFlowLog:@"调用 show"];
    } else {
