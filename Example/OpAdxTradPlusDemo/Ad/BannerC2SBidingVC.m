...
static NSString * const kBannerC2SPlacementId = @"237C7486C2182C5057A98D8BDA2BA822";
static NSString * const kOpAdxTradPlusC2SPriceEventNotification = @"OpAdxTradPlusC2SPriceEventNotification";

@interface BannerC2SBidingVC ()<TradPlusADBannerDelegate>
...
- (void)viewDidLoad {
    ...
    loadBtn.frame = CGRectMake(20, 150, 100, 44);
    ...
    showBtn.frame = CGRectMake(140, 150, 100, 44);
    [showBtn addTarget:self action:@selector(showAd) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:showBtn];

    _containerView = [[UIView alloc] initWithFrame:CGRectMake((self.view.bounds.size.width - 300) / 2.0, 210, 300, 250)];
    _containerView.backgroundColor = [UIColor colorWithWhite:0.95 alpha:1];
    [self.view addSubview:_containerView];

    _flowTextView = [[UITextView alloc] initWithFrame:CGRectMake(20, 470, self.view.bounds.size.width - 40, self.view.bounds.size.height - 490)];
...
- (void)loadAd {
    ...
    [self.banner setAdUnitID:kBannerC2SPlacementId];
    self.banner.delegate = self;
    self.banner.frame = self.containerView.bounds;
    [self.containerView addSubview:self.banner];
    [self appendFlowLog:@"点击 Load，开始请求竞价 Banner"];
    [self.banner loadAdWithSceneId:nil];
}

- (void)showAd {
    [self appendFlowLog:[NSString stringWithFormat:@"点击 Show，isAdReady=%@", self.banner.isAdReady ? @"YES" : @"NO"]];
    if (self.banner.isAdReady) {
        [self.banner showWithSceneId:nil];
        [self appendFlowLog:@"调用 showWithSceneId:nil"];
    } else {
...
- (void)tpBannerAdLoadFailWithError:(NSError *)error {
    self.logLabel.text = [NSString stringWithFormat:@"Banner 加载失败: %@", error.localizedDescription];
